...
namespace Ch09
{
    class Member
    {
        ...
        public Member(string uid, string name, int age)        //생성자
        {
            this.uid = uid;
            this.name = name;
            this.age = age;
        ...
        public string Uid { get => uid; set => uid = value; }      //getter, setter
        public string Name { get => name; set => name = value; }   //빠른작업리펙터링-필드캡슐화(그리고속성을사용함)
    ...
    internal class _3_Linq
    {
        ...
        {
            // 컬렉션 생성
            List<int> dataset = new List<int> { 1, 2, 3 };
            ...
            // 예제. 데이터가 5이상 큰 순서대로 출력
            var result4 = from n in dataset    //from절과 where절은 반드시 붙어있어야하고,
                          where n >= 5
                          orderby n descending //내림차순(큰순서대로)
                          select n;            //select절은 반드시 마지막.
            ...
            var result5 = from member in members 
                          where member.Age >= 30 
                          orderby member.Name ascending //오름차순(사전순서대로)
                          select member;
